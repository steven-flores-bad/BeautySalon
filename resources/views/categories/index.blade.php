...
<head>
    ...
    <title>Categorías - Salón de Belleza</title>
    <!-- Tailwind CSS CDN -->
    <script src="https://cdn.tailwindcss.com"></script>
    <!-- Alpine.js para interactividad de modales -->
...
<body class="bg-gray-50 text-gray-800 font-sans antialiased" x-data="{ openCreateModal: false, openEditModal: false, editForm: {} }">
    ...
    <div class="min-h-screen flex flex-col">
        ...
        <main class="flex-grow max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                <h1 class="text-2xl font-bold text-gray-900 w-full">Listado de Categorías</h1>

                <form method="GET" action="{{ route('categories.index') }}" class="w-full sm:w-85">
                        ...
                        <thead>
                            <tr class="bg-gray-100/70 text-gray-600 uppercase text-xs tracking-wider border-b border-gray-200">
                                <th class="py-3 px-4 font-semibold">
                                    <a href="{{ route('categories.index', array_merge(request()->query(), ['sort' => 'nombre', 'direction' => ($sort === 'nombre' && $direction === 'asc') ? 'desc' : 'asc'])) }}"
                                ...
                                <th class="py-3 px-4 font-semibold">
                                    <a href="{{ route('categories.index', array_merge(request()->query(), ['sort' => 'descripcion', 'direction' => ($sort === 'descripcion' && $direction === 'asc') ? 'desc' : 'asc'])) }}"
                                       class="flex items-center gap-1 hover:text-gray-900 transition">
                                        Descripción
                                        @if ($sort === 'descripcion')
                                            <span class="text-pink-600">{{ $direction === 'asc' ? '▲' : '▼' }}</span>
                        ...
                        <tbody class="divide-y divide-gray-200 text-sm">
                            @forelse ($categories as $category)
                                <tr class="hover:bg-gray-50/50 transition">
                                    <td class="py-3 px-4 text-gray-900 font-semibold">{{ $category->nombre }}</td>
                                    <td class="py-3 px-4 text-gray-600">{{ $category->descripcion }}</td>
                                    ...
                                    <td class="py-3 px-4 text-right space-x-2">
                                        <!-- Botón Editar -->
                                        <button @click="openEditModal = true; editForm = {{ Illuminate\Support\Js::from($category) }}"
                                                class="text-indigo-600 hover:text-indigo-900 font-medium text-xs bg-indigo-50 px-2.5 py-1 rounded-md transition">
                                            Editar
                                        </button>
                                        <!-- Botón Eliminar -->
                                        <form action="{{ route('categories.destroy', $category->id) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de eliminar esta categoría?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-8 text-gray-400">No hay categorías registradas todavía.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <form action="{{ route('categories.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="nombre" required class="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-pink-500 focus:border-pink-500">
                    </div>
                    <div>
    ...
    <div x-show="openEditModal" class="fixed inset-0 z-50 overflow-y-auto bg-black/50 flex items-center justify-center p-4" x-cloak>
        <div class="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl relative" @click.away="openEditModal = false">
            <h3 class="text-lg font-bold text-gray-900 mb-4">Editar Categoría</h3>
            <form :action="'/categories/' + editForm.id" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-4">
                    <div>
